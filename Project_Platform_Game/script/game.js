 ...
 * Levels
 * ------
 * Periods are empty space, hash (#) characters are walls, and + signs are lava. The player’s starting
...
//EXERCISES
//EX 1.GAME OVER
// async function because of the await expression. it pauses and resumes and waits for the 'Promise' returned by the'await' expr
async function runGame(plans, Display) { // level plans and display class
    let lives = 3; // Set the initial number of lives to 3

    for (let level = 0; level < plans.length && lives > 0;) { // iterates through each level in the plans array
        console.log(`You have ${lives} lives left.`); // Output the number of lives before starting a level
        let status = await runLevel(new Level(plans[level]), Display); // await keyword for pausing the execution of the runGame function
        if (status == "lost") {
            lives--; // Subtract one life if the player dies
        } else if (status == "won") {
            level++;
        }
    }

    if (lives > 0) { // at the end of the game loop we check the player's lives
        console.log("You've won!");
    } else {
        console.log("Game over!");
    }
}
// lives variable that stores the number of lives the player has left, and we initialized it to 3.
// Before starting each level, we output the current number of lives to the console
//
// Inside the game loop, we check the status variable returned by the runLevel function.
// If the player lost the level, we subtract one life from the lives variable.
// If the player won the level, we move on to the next level.
//
// At the end of the game loop, we check if the player still has lives left.
// If they do, we output "You've won!" to the console. If they don't, we output "Game over!" instead.


//EX 2. PAUSING THE GAME
// New function to register/unregister keyboard event handlers
...
function runLevel(level, Display) {
    ...
    return new Promise(resolve => {
        function escHandler(event) {
            if (event.key != "Escape") return; // checks whether the key pressed is the ESC , if so it prevents the default behaviour ( closing de window for ex)
            event.preventDefault();
            if (running == "no") { // if running is 'no' it sets it to 'yes' and starts the animation
                running = "yes";
                runAnimation(frame);
            } else if (running == "yes") { // if 'yes' it sets it to 'pause'
                running = "pausing";
            } else {
                running = "yes";
            }
        }
        window.addEventListener("keydown", escHandler); // window object registers an event listener for the keydown event , which will
        //call the escHandler whenerver a key is pressed
        let arrowKeys = trackKeys(["ArrowLeft", "ArrowRight", "ArrowUp"]); // checks the arrow keys if currently pressed down

        function frame(time) { // callback function that will be called repeatedly by the runAnimation function in order to animate the game
            if (running == "pausing") {
                running = "no";
                return false; // pause animation
            }

            state = state.update(time, arrowKeys);
            display.syncState(state); // checks whether the game is still being played
            if (state.status == "playing") {
        ...
        runAnimation(frame); // animation loop by calling frame repeatedly with elapsed time as an argument until frame returns false
    });
}

function trackKeys(keys) {
    let down = Object.create(null); // it takes an array of key names as input and an object that represents the state of those keys.

    function track(event) { // checks if the key that triggered the event is included in the keys array
        if (keys.includes(event.key)) {
            down[event.key] = event.type == "keydown"; // if it is the down object is updated with the current state of the key
            // true if the key was pressed or false if the key was released
            event.preventDefault(); // prevent any default behaviour of the associated key which is pressed
        }
    }
    window.addEventListener("keydown", track); // call the track func for each events: keyup/keydown
    window.addEventListener("keyup", track);
    down.unregister = () => { // removes both event listeners: keyup/down
        window.removeEventListener("keydown", track);
        window.removeEventListener("keyup", track);
    };
    return down; // returns down object
}
...
//EX 3 . MONSTER
        ...
        get type() { return "monster"; } // getter method type that returns the string monster

        static create(pos) { // takes a position vector as argument and returnes a monster obj
        return new Monster(pos.plus(new Vec(0, -1)), new Vec(2, 0)); // returns the enemy with given position and fixed speed vector
    }

        update(time, state) { // update method that updates the monster's position based on its speed and the elapsed time
        let newPos = this.pos.plus(this.speed.times(time)); // new position of the monster obj
        if (!state.level.touches(newPos, this.size, "wall")) {
        return new Monster(newPos, this.speed); // checks if there woul be a collision with a wall at its new position and returns a new monster obj with updated
            // position and speed
    } else {
        return new Monster(this.pos, this.speed.times(-1)); // if the monster collides with a wall , it returns a new monster object with the same
            // position but with negated speed vector , so that the monster bounces back in the opposite direction
    }
    }

        collide(state) {
        let player = state.player; // checks if the player is on top of the monster or not( player bottom with monster top)
        if (player.pos.y + player.size.y < this.pos.y + 0.5) {
        // Player is on top, remove monster
        return new State(state.level, state.actors.filter(a => a != this));
    } else {
        // Player is hit, game is lost
        return new State(state.level, state.actors, "lost"); // returns a new state object with the same level and actors but with status "lost"
    }
    }
    }

        Monster.prototype.size = new Vec(1.2, 2); // fixed size vector for all monster objects.

        levelChars["M"] = Monster; // add the monster class to the levelChars object which maps characters in the level definition string to the corresponding
     // object types
